...
import { useUserGetSignal } from "~/queries/user";
...
const { data, isPending, isError, error } = useUserGetSignal(query);
</script>

<template>
  <symbol-filter v-model:filter="filter" />
  <signals-table :data="data || []" :is-pending="isPending" :is-error="isError" :error="error"
