...
import type { IStore, IWssAdapter, IServiceConfig } from './types'
...
const receiveHandler = (event: { data: string }) => {
  ...
  if (
    ...
    const error = response.code || response.params?.success === false || response.params?.error
...
function onError(response: IResponse) {
  ...
  const errorMsg = params.reason || params.error || response.params || error?.message || response.code
