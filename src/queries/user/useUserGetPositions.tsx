...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 2) {
    const response = await api.app.UserGetS2Position<ApiResponse, ApiParams>(params)
    return { data: response.params.data }
  }
  const response = await api.app.UserGetPosition<UserGetPositionResponse, ApiParams>(params)
  return { data: response.params.data }
}
