...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    if (params.debug) {
      const response = await api.app.UserGetDebugEvent1<UserGetDebugEvent1Response, ApiParams>(
        params,
      )
    ...
    const response = await api.app.UserGetEvent1<UserGetEvent1Response, ApiParams>(params)
  ...
  if (params?.strategyId === 2) {
    if (params.debug) {
      const response = await api.app.UserGetS2DebugEvent<UserGetS2DebugEventResponse, ApiParams>(
        params,
      )
      const sortedData = sortData(response.params.data.slice(-4000))
      return { events: sortedData }
    }
    const response = await api.app.UserGetS2Event<UserGetS2EventResponse, ApiParams>(params)
    const sortedData = sortData(response.params.data)
    return { events: sortedData }
  }
  ...
  if (params?.strategyId === 3) {
    if (params.debug) {
      const response = await api.app.UserGetS3DebugEvent<UserGetS3DebugEventResponse, ApiParams>(
        params,
      )
      const sortedData = sortData(response.params.data.slice(-4000))
      return { events: sortedData }
    }
    const response = await api.app.UserGetS3Event<UserGetS3EventResponse, ApiParams>(params)
    const sortedData = sortData(response.params.data)
    return { events: sortedData }
  }
  ...
  if (params?.strategyId === 4) {
    if (params.debug) {
      const response = await api.app.UserGetS4DebugEvent<UserGetS4DebugEventResponse, ApiParams>(
        params,
      )
      const sortedData = sortData(response.params.data.slice(-4000))
      return { events: sortedData }
    }
    const response = await api.app.UserGetS4Event<UserGetS4EventResponse, ApiParams>(params)
    const sortedData = sortData(response.params.data)
    return { events: sortedData }
  }
