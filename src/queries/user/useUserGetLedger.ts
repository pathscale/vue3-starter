...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    const response = await api.app.UserGetS1Ledger<ApiResponse, ApiParams>(params)
    return { orders: response.params.data }
  } else if (params?.strategyId === 2) {
    const response = await api.app.UserGetS2Ledger<UserGetS2LedgerResponse, ApiParams>(params)
    return { orders: response.params.data }
  }
  const response = await api.app.UserGetLedger<UserGetLedgerResponse, ApiParams>(params)
  return { orders: response.params.data }
}
