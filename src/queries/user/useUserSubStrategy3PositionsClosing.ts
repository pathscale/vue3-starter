...
const getFn = async (params?: ApiParams) => {
  const response = await api.app.UserSubStrategy3PositionsClosing<
    ApiResponse,
    ApiParams
  >(params);
...
export const unsubscribeUserSubStrategy3PositionsClosing = () =>
  getFn({ unsubscribe: true });

export const updateUserSubStrategy3PositionsClosing = (
