...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    const response =
      await api.app.UserGetLivePosition1<UserGetLivePosition1Response>(params);
    return { data: response.params.data };
  }
  if (params?.strategyId === 2) {
    const response =
      await api.app.UserGetS2LivePosition<UserGetS2LivePositionResponse>(
        params,
      );
    return { data: response.params.data };
  }
  const response =
    await api.app.UserGetStrategyLivePosition<UserGetStrategyLivePositionResponse>(
      params,
    );
  return { data: response.params.data };
};
