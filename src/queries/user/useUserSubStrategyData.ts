...
import type {
  UserSubPrice0Response as StreamResponse,
  UserSubPrice0Params,
} from "~/models/user";
...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    const response = await api.app.UserSubPriceDifference<
      ApiResponse,
      ApiParams
    >(params);
  ...
  if (params?.strategyId === 2) {
    const response = await api.app.UserSubBestBidAskAcrossExchanges<
      ApiResponse,
      ApiParams
    >(params);
    return response.params;
