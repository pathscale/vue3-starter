...
const getFn = async (params?: ApiParams) => {
  const response = await api.app.UserSubStrategy3PositionsOpening<ApiResponse, ApiParams>(params)
  return { positions: response.params.data }
}

export const unsubscribeUserSubStrategy3PositionsOpening = () => getFn({ unsubscribe: true })

export const updateUserSubStrategy3PositionsOpening = (payload: StreamResponse) => {
