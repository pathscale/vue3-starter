...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    const response = await api.app.UserGetOrder1<
      UserGetOrder1Response,
      ApiParams
    >(params);
  ...
  if (params?.strategyId === 2) {
    const response = await api.app.UserGetS2Order<
      UserGetS2OrderResponse,
      ApiParams
    >(params);
    return { orders: response.params.data };
  }
  const response = await api.app.UserGetOrder<UserGetOrderResponse, ApiParams>(
    params,
  );
  return { orders: response.params.data };
};
