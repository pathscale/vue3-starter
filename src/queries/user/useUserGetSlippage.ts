...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    const response = await api.app.UserGetSlippage1<ApiResponse, ApiParams>(
      params,
    );
    return { data: response.params.data };
  }
  if (params?.strategyId === 2) {
    const response = await api.app.UserGetS2Slippage<
      UserGetS2SlippageResponse,
      ApiParams
    >(params);
    return { data: response.params.data };
  ...
  if (params?.strategyId === 3) {
    const response = await api.app.UserGetS3Slippage<
      UserGetS3SlippageResponse,
      ApiParams
    >(params);
    return { data: response.params.data };
