...
const getFn = async (params?: ApiParams) => {
  if (params?.strategyId === 1) {
    const response = await api.app.UserGetSignal1<UserGetSignal1Response, ApiParams>(params)
    return response.params.data
  } else if (params?.strategyId === 2) {
    const response = await api.app.UserGetS2Signal<UserGetS2SignalResponse, ApiParams>(params)
    return response.params.data
  } else if (params?.strategyId === 3) {
    const response = await api.app.UserGetS3Signal<UserGetS3SignalResponse, ApiParams>(params)
    return response.params.data
  } else if (params?.strategyId === 4) {
    const response = await api.app.UserGetS4Signal<UserGetS4SignalResponse, ApiParams>(params)
    return response.params.data
  }
