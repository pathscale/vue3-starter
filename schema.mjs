...
const prettierrc = JSON.parse(await readFile(".prettierrc.json", "utf-8"));
...
schema.services.forEach((service) => {
    ...
    if (typeof rawSchema?.ty === "string") {
      ...
      if (typeof rawSchema.ty.Optional.Vec === "string") {
        ...
        rawSchema.ty.Optional.Struct.fields.forEach((subRawSchema) => {
          schemaItems.properties[snakeToCamelCase(subRawSchema.name)] =
            loopParse(subRawSchema);
        });
      }
      ...
      rawSchema.ty.Optional.DataTable.fields.forEach((subRawSchema) => {
        schemaItems.items.properties[snakeToCamelCase(subRawSchema.name)] =
          loopParse(subRawSchema);
  ...
  fs.writeFileSync(
    ...
    service.endpoints
      .map((endpoint) => `export * from './${endpoint.name}Dto';`)
      .join("\r\n"),
  );

  fs.writeFileSync(
    `${apiBasePath}/${service.name}.json`,
